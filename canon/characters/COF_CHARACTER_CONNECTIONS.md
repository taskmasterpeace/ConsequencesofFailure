...
```mermaid
graph TB
    GRAYS[The Grays] --> RING[Ring Entity]
    GRAYS --> ALLIANCE[Alliance of Four]

    ALLIANCE --> IE[Iron Eaters]
    ALLIANCE --> SL[The Slaught]
    ALLIANCE --> DP[The Deep]
    ALLIANCE --> FR[Fourth Race - Unknown]

    RING --> BW[Black Woman Jackal]
    BW --> JACKALS[Jackal Network]
    JACKALS --> LSW[Hunts LSWs]

    SPEAR[SPEAR - Global] --> GALLOWAY[Galloway]
    SPEAR --> SPAHIRE[Spahire]
    SPEAR --> RUSTY[Rusty Richards?]
...
```mermaid
graph LR
    L5[Level 5 - Existential] --> ASHA_L[Asha - Time Travel]
    L5 --> MWAN_L[Mwangaza † - Time + Mind Control]

    L4[Level 4 - Reality-Altering] --> UNKNOWN_L4[Unknown Characters]

    L3[Level 3 - Multiple Powers] --> E24_L[Most E24 Members]
    L3 --> EZIOBI_L[Eziobi Likely]

    L2[Level 2 - Single Power] --> MAJORITY[Majority of LSWs]

    L1[Level 1 - Enhanced] --> BASIC[Enhanced Humans]
```
...
```mermaid
graph TB
    GLOBAL[SPEAR - Global Command] --> USA_OPS[USA Operations]
    GLOBAL --> INDIA_OPS[India Coordination]
    GLOBAL --> CHINA_OPS[China Coordination]
    GLOBAL --> AFRICA_OPS[Africa Coordination]

    USA_OPS --> DECK52[Deck 52]
    USA_OPS --> GALLOWAY_OP[Galloway/Spahire Team]

    INDIA_OPS --> E24[Establishment 24]
    E24 --> RAGHAVAN_T[Raghavan's Team]

    CHINA_OPS --> LIUXIAO_OP[Liu Xiao Unit]

    AFRICA_OPS --> EZIOBI_OP[Eziobi/Kaiser Unit]
```
...
timeline
    ...
    Season 6 : Time Traveler Arrives (Asha from Future?)
    Season 7 : Final Preparation : All Forces Unite
    Season 8 : Mass Invasion : All Characters in Play
```
